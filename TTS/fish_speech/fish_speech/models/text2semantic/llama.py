...
from TTS.fish_speech.fish_speech.conversation import SEMANTIC_TOKEN
from TTS.fish_speech.fish_speech.utils import RankedLogger
...
class BaseModelArgs:
    ...
    def from_pretrained(path: str):
        ...
        # Ensure we're looking for "config.json" explicitly
        config_path = "/Users/tommyfalkowski/Code/READ2ME/TTS/fish_speech/checkpoints/models--fishaudio--fish-speech-1.4/snapshots/069c573759936b35191d3380deb89183c0656f59/config.json"
        # config_path = path / "config.json" if path.is_dir() else path

        # Debugging output to confirm we're opening config.json
        print(f"Opening config file at: {config_path}")

        # # Ensure the configuration file exists
        # if not config_path.exists() or config_path.suffix != ".json":
        #     raise FileNotFoundError(
        #         f"Configuration file 'config.json' not found at {config_path}"
        #     )
