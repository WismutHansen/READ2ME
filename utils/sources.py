        ...
        for source_url in source_urls:
            ...
            for article_url in article_urls:
                try:
                    ...
                    # Create a modified URL for keyword matching (replacing hyphens with spaces)
                    modified_article_url = article_url.replace('-', ' ').lower()
                    print(f"Processing modified article URL: {modified_article_url}")

                    # Check if any keyword pattern is present in the modified article URL
                    matched = False
                    for pattern in keyword_patterns:
                        if pattern.search(modified_article_url):
                            matched = True
                            print(f"Keyword '{pattern.pattern}' found in URL: {article_url}")
                            add_task('url', article_url, 'edge_tts')  # Use original URL with hyphens
                            break
