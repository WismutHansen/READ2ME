...
def infer_batch(
    ...
    for i, gen_text in enumerate(tqdm.tqdm(gen_text_batches)):
        ...
        zh_pause_punc = r"。，、；：？！"
        transcript_len = len(transcript.encode("utf-8")) + 3 * len(re.findall(zh_pause_punc, transcript))
        gen_text_len = len(gen_text.encode("utf-8")) + 3 * len(re.findall(zh_pause_punc, gen_text))
        duration = abs_voice_path_len + int(abs_voice_path_len / transcript_len * gen_text_len / speed)
...
def infer(abs_voice_path, transcript, gen_text, cross_fade_duration=0.15):
    ...
    max_chars = int(len(transcript.encode("utf-8")) / (audio.shape[-1] / sr) * (25 - audio.shape[-1] / sr))
    gen_text_batches = chunk_text(gen_text, max_chars=max_chars)
    print("transcript", transcript)
    for i, gen_text in enumerate(gen_text_batches):
        print(f"gen_text {i}", gen_text)

    print(f"Generating audio using F5-TTS in {len(gen_text_batches)} batches, loading models...")
    return infer_batch(
        (audio, sr),
        transcript,
        gen_text_batches,
...
def transcribe_audio(audio_path: str) -> str:
    ...
    if not transcription.strip():  # If transcription is empty, proceed with transcription
        print("No reference text provided, transcribing reference audio...")
